...
class UiFormBuilder < ActionView::Helpers::FormBuilder
  # Maps a stock field method to the class it should default to.
  FIELD_CLASSES = {
  ...
  # Returns a copy of `options` with `default_class` placed before any
  # caller-supplied class, so callers can still add layout utilities.
